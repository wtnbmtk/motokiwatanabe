<script lang="ts">
  ...
  let x1 = 20; // 固定始点
  let y1 = 0; // 固定始点
  let x2 = 20; // 終点の初期位置は始点の下
  let y2 = 80; // 終点の初期位置
  const initialY2 = y2; // y2の初期位置を保存
  // biome-ignore lint/style/useNamingConvention: <explanation>
  const maxXOffset = 10; // x軸の許容範囲
  let isDragging = false;
  let animationFrame = 0;
  const maxDistance = 100; // 始点と終点の最大距離
  let endCircleRadius = 8; // 終点のクリック可能な領域の半径
  let bulbArea1: string;
  let bulbArea2: string;
  let bulbArea3: string;
  ...
  function handleMove(event: MouseEvent | TouchEvent) {
    ...
    x2 = Math.min(Math.max(cursorPoint.x, x1 - maxXOffset), x1 + maxXOffset);
...
<div class="lighting">
  <svg id="bulb">
    <defs>
      <!-- 電球アイコン -->
      <symbol id="bulb-icon" viewBox="0 0 11 16">
        <path
          fill={bulbArea1}
          d="M 5.3339844,0 C 2.393454,0 0,2.103589 0,4.6894531 0,6.0574518 0.8981592,7.4670597 1.765625,8.828125 2.513091,9.9995905 3.2262856,11.117712 3.4492188,12.267578 H 7.2167969 C 7.4402634,11.117445 8.1531914,9.9993238 8.9003906,8.828125 9.7683898,7.4670597 10.666016,6.0574518 10.666016,4.6894531 10.666016,2.103589 8.2745148,0 5.3339844,0 Z"
        />
        <path
          fill={bulbArea2}
    ...
    <use href="#bulb-icon" x="0.3" y="0" class="bulb" />
  </svg>
  <svg
    id="string"
    role="img"
...
<style>
  ...
  :global(#bulb-icon > path:nth-child(2)) {
    fill: black;
  }
  :global(#bulb-icon > path:nth-child(3)) {
    fill: white;
  }
  :global(#logo) {
    fill: white;
  ...
  :global(.theme-dark #bulb-icon > path:nth-child(2)) {
    fill: black;
  }
  :global(.theme-light #bulb-icon > path:nth-child(2)) {
    fill: black;
  }
  :global(.theme-dark #bulb-icon > path:nth-child(3)) {
    fill: white;
  }
  :global(.theme-light #bulb-icon > path:nth-child(3)) {
    fill: black;
  }
